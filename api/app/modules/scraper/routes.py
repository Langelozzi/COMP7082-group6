...
def post_build_dom_tree(req: BuildDomTreeRequest, current_user: AuthUser = Depends(require_auth)) -> ScrapeGoatDOMTree:
    return build_dom_tree(req.url)


@router.post("/scrape")
def post_scrape(config: ScrapeConfig, current_user: AuthUser = Depends(require_auth)) -> ScrapedDataset:
